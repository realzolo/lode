...
_REQUIRED_ROLE_DATA_CLASSES = {
    role: (
        ModelDataClass.SOURCE_CODE.value
        if role == "resource_analyst"
        else ModelDataClass.MASKED.value
    )
...
def _model_role_coverage(
    ...
    data_class_eligible_roles = {
        role
        for binding in eligible_bindings
        for role in binding.allowed_roles
        if _REQUIRED_ROLE_DATA_CLASSES.get(role) in binding.allowed_data_classes
    }
    return roles, data_class_eligible_roles
...
async def _workspace_readiness(
    ...
    if policy is None:
        ...
        roles, data_class_eligible_roles = _model_role_coverage(bindings, expected_revisions)
        missing_roles = sorted(_REQUIRED_MODEL_ROLES - roles)
        missing_data_class_roles = sorted(
            (_REQUIRED_MODEL_ROLES & roles) - data_class_eligible_roles
        )
        model_details = {
            "policy_revision": policy.revision,
            "eligible_binding_count": len(bindings),
            "missing_roles": missing_roles,
            "required_data_classes": _REQUIRED_ROLE_DATA_CLASSES,
