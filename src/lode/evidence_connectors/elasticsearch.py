"""Elasticsearch connector with Elasticsearch-only version proof."""
...
class ElasticsearchConnector(SearchConnectorMechanics):
    kind = "elasticsearch"
    ...
    def __init__(
        ...
        self.config = ElasticsearchConnectorConfig.model_validate(config)
    ...
    async def verify(self) -> VerificationResult:
        ...
        if (
            match is None
            or int(match.group("major")) not in {8, 9}
