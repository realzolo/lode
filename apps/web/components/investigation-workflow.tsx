...
import { Bot, ChevronDown, CircleDot, Clock3, Database, FileSearch, Wrench } from 'lucide-react';
...
export function InvestigationWorkflow({ detail, selectedOperationId, onSelectOperation, onOpenEvidence }: { detail: InvestigationDetail; selectedOperationId: string | null; onSelectOperation: (id: string) => void; onOpenEvidence: (id: number) => void }) {
  ...
  return <div className="serial-workflow" aria-label="串行调查步骤">
    {detail.steps.map((step) => <section className={`serial-step serial-step-${step.status}`} key={step.id}>
      ...
      <div className="serial-step-body">
        ...
        <div className="serial-operations">{(byStep.get(step.db_id) || []).map((operation) => <Operation key={operation.id} operation={operation} selected={selectedOperationId === operation.id} onSelect={() => onSelectOperation(operation.id)} onOpenEvidence={onOpenEvidence} />)}</div>
...
function Operation({ operation, selected, onSelect, onOpenEvidence }: { operation: InvestigationOperation; selected: boolean; onSelect: () => void; onOpenEvidence: (id: number) => void }) {
  return <details className={`serial-operation${selected ? ' is-selected' : ''}`} open={selected || operation.status === 'running'}>
    ...
    <div className="serial-operation-detail">
      ...
      {operation.evidence_refs.length > 0 && <section><span>证据</span><div className="serial-evidence-links">{operation.evidence_refs.map((ref) => <button type="button" key={ref} onClick={() => onOpenEvidence(ref)}><FileSearch size={12} />证据 {ref}</button>)}</div></section>}
    </div>
  </details>;
}
