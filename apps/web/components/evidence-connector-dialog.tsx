...
type LokiCondition = { kind: 'condition'; label: string; operator: LokiOperator; value: string };
type LokiGroup = { kind: 'group'; combinator: 'all' | 'any'; items: Array<LokiCondition | LokiGroup> };
type LokiFilterPayload = { kind: 'group'; combinator: 'all' | 'any'; items: Array<LokiConditionPayload | LokiFilterPayload> };
type LokiConditionPayload = { kind: 'condition'; label: string; operator: LokiOperator; values: string[] };

const emptyCondition = (): LokiCondition => ({ kind: 'condition', label: '', operator: 'equals', value: '' });
const initialFilter = (): LokiGroup => ({ kind: 'group', combinator: 'all', items: [emptyCondition()] });
const splitValues = (value: string) => value.split(/[\n,]/).map((item) => item.trim()).filter(Boolean);
const toLokiFilterPayload = (group: LokiGroup): LokiFilterPayload => ({
  ...group,
  items: group.items.map((item): LokiConditionPayload | LokiFilterPayload => item.kind === 'condition'
    ? { ...item, values: splitValues(item.value) }
    : toLokiFilterPayload(item)),
});
const hasPositiveMatcher = (item: LokiCondition | LokiGroup): boolean => item.kind === 'condition'
  ? ['equals', 'any_of'].includes(item.operator) && Boolean(item.label.trim() && splitValues(item.value).length)
  : item.items.some(hasPositiveMatcher);
...
export function EvidenceConnectorDialog({ open, onOpenChange, workspaceId, kinds, onCreated }: { open: boolean; onOpenChange: (open: boolean) => void; workspaceId: string; kinds: Array<{ kind: string }>; onCreated: () => Promise<void> }) {
  ...
  async function create() {
    ...
    try {
      const created = await createConnector(workspaceId, {
        ...
        ...(kind === 'loki' ? { tenant_id: tenantId || undefined, root_filter: toLokiFilterPayload(rootFilter), authentication, credential: authentication === 'none' ? undefined : credential } : {}),
  ...
  const lokiValid = kind !== 'loki' || hasPositiveMatcher(rootFilter);
...
function LokiGroupEditor({ group, depth, onChange, removable = false, onRemove }: { group: LokiGroup; depth: number; onChange: (group: LokiGroup) => void; removable?: boolean; onRemove?: () => void }) {
  ...
  return <div className="space-y-2 border p-3"><div className="flex items-center justify-between gap-2"><Select className="w-28" value={group.combinator} onChange={(event) => onChange({ ...group, combinator: event.target.value as 'all' | 'any' })}><option value="all">{t('matchAll')}</option><option value="any">{t('matchAny')}</option></Select>{removable ? <Button size="icon" variant="ghost" title={t('removeGroup')} aria-label={t('removeGroup')} onClick={onRemove}><Trash2 size={15} /></Button> : null}</div>{group.items.map((item, index) => item.kind === 'condition' ? <div key={index} className="grid gap-2 sm:grid-cols-[1fr_140px_1fr_32px]"><Input placeholder={t('labelName')} value={item.label} onChange={(event) => update(index, { ...item, label: event.target.value })} /><Select value={item.operator} onChange={(event) => update(index, { ...item, operator: event.target.value as LokiOperator })}><option value="equals">{t('equals')}</option><option value="not_equals">{t('notEquals')}</option><option value="any_of">{t('anyOf')}</option><option value="not_any_of">{t('notAnyOf')}</option></Select><Input placeholder={t('labelValues')} value={item.value} onChange={(event) => update(index, { ...item, value: event.target.value })} /><Button size="icon" variant="ghost" title={t('removeCondition')} aria-label={t('removeCondition')} disabled={group.items.length === 1} onClick={() => remove(index)}><Trash2 size={15} /></Button></div> : <LokiGroupEditor key={index} group={item} depth={depth + 1} removable onRemove={() => remove(index)} onChange={(value) => update(index, value)} />)}<div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => onChange({ ...group, items: [...group.items, emptyCondition()] })}><Plus size={14} />{t('addCondition')}</Button>{depth < 3 ? <Button size="sm" variant="outline" onClick={() => onChange({ ...group, items: [...group.items, { kind: 'group', combinator: 'all', items: [emptyCondition()] }] })}><Plus size={14} />{t('addGroup')}</Button> : null}</div></div>;
}
