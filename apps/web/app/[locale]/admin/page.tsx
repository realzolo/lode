    ...
    <div className="operational-table">
      <div className="table-wrap"><table className="table"><thead><tr><th>{t('name')}</th><th>{t('kafkaTopic')}</th><th>{t('policy')}</th><th>{t('investigationDepth')}</th><th>{t('ingestion')}</th><th>{t('updated')}</th><th /></tr></thead>
        <tbody>{rows.map((row) => <tr key={row.id}>
          ...
          <td><div className="flex justify-end gap-1"><Button size="icon" variant="ghost" aria-label={t('ingestion')} title={t('ingestion')} onClick={() => void transition(row)}>{row.ingestion_state === 'active' ? <CirclePause size={16} /> : <CirclePlay size={16} />}</Button><Button size="icon" variant="ghost" asChild><Link href={`/admin/workspaces/${row.id}`} aria-label={tc('open')} title={tc('open')}><ArrowUpRight size={16} /></Link></Button></div></td>
        </tr>)}</tbody></table></div>
      {!loading && rows.length === 0 && <p className="p-8 text-center text-muted-foreground">{t('noWorkspaces')}</p>}
      {loading && <p className="p-8 text-center text-muted-foreground">{tc('loading')}</p>}
    </div>
    <Dialog open={open} onOpenChange={setOpen}><DialogContent><DialogHeader><DialogTitle>{t('newWorkspace')}</DialogTitle></DialogHeader><div className="space-y-4"><label className="field"><span className="field-label">{t('name')}</span><Input value={name} onChange={(event) => setName(event.target.value)} /></label><label className="field"><span className="field-label">{t('kafkaTopic')}</span><Input className="mono" value={topic} onChange={(event) => setTopic(event.target.value)} /></label><label className="field"><span className="field-label">{t('initialPosition')}</span><Select value={position} onChange={(event) => setPosition(event.target.value as 'earliest' | 'latest')}><option value="latest">{t('latest')}</option><option value="earliest">{t('earliest')}</option></Select></label></div><DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>{tc('cancel')}</Button><Button variant="primary" disabled={!name.trim() || !topic.trim()} onClick={() => void create()}>{t('createWorkspace')}</Button></DialogFooter></DialogContent></Dialog>
  </main>;
}
