...
import { archiveInvestigation, fetchInvestigation, fetchInvestigationAudit, fetchInvestigationTechnical, openInvestigationStream, retryInvestigation } from '@/lib/api';
...
export default function InvestigationPage({ params }: { params: { investigationId: string } }) {
  ...
  const loadAudit = useCallback(async (append = false, afterId = 0) => {
    try {
  ...
  useEffect(() => {
    ...
    void fetchInvestigationTechnical(params.investigationId).then(setTechnical).catch((cause) => setError(String(cause)));
  ...
  async function retry() {
    try {
      const created = await retryInvestigation(investigation.public_id);
  ...
  async function archive() {
    try {
      await archiveInvestigation(investigation.public_id);
      await load();
    } catch (cause) {
      setError(String(cause));
    }
    ...
    <header className="border-b pb-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 mono text-xs text-muted-foreground">{detail.public_id}</p>
        </div>
...
function Timeline({ items, dateLocale }: { items: InvestigationOverview['timeline']; dateLocale: string }) {
  const t = useTranslations('workbench');
  return <div className="divide-y border-y">{items.map((item) => <article key={item.ordinal} className="grid gap-3 py-4 md:grid-cols-[48px_1fr_180px]"><div className="flex h-8 w-8 items-center justify-center rounded-sm border font-mono text-xs">{item.ordinal}</div><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-medium">{item.purpose}</h3><span className="text-xs text-muted-foreground">{item.kind}</span></div><p className="mt-1 text-sm text-muted-foreground">{item.expected_evidence}</p>{item.failure_code && <p className="mt-2 text-xs text-destructive">{item.failure_code}</p>}</div><div className="text-xs text-muted-foreground md:text-right"><div>{statusLabel(item.status, t)}</div><div className="mt-1">{item.finished_at ? new Date(item.finished_at).toLocaleString(dateLocale) : '-'}</div></div></article>)}{items.length === 0 && <p className="py-8 text-center text-muted-foreground">{t('noTimeline')}</p>}</div>;
}
...
function Evidence({ items, dateLocale }: { items: InvestigationOverview['evidence']; dateLocale: string }) {
  ...
  return <div className="operational-table"><div className="table-wrap"><table className="table"><thead><tr><th>{t('evidenceId')}</th><th>{t('evidenceKind')}</th><th>{t('evidenceClass')}</th><th>{t('sourceRevision')}</th><th>{t('time')}</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td>{item.id}</td><td>{item.kind}</td><td>{item.evidence_class}</td><td className="mono text-xs">{item.source_revision || '-'}</td><td className="text-xs">{item.source_time_start ? new Date(item.source_time_start).toLocaleString(dateLocale) : '-'}</td></tr>)}</tbody></table></div></div>;
}
