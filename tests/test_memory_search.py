"""Hermetic tests for pgvector semantic shared memory (M5).
...
memory lookup decision logic are tested directly.
"""
...
from lode.engine.tools import get_memory


class _Mem:
    """Minimal stand-in for an ORM Memory row."""
...
class _FakeSession:
    """Returns a fixed exact-match memory (or None) for the exact path."""

    def __init__(self, exact_memory=None, rows=None) -> None:
        self._exact = exact_memory
...
# --- get_memory decision logic -------------------------------------------


async def test_get_memory_exact_override():
    mem = _Mem(42, "recorded conclusion")
    sess = _FakeSession(exact_memory=mem)
    res = await get_memory(sess, 1, dedupe_key="abc123")
...
async def test_get_memory_semantic_match_within_threshold():
    mem = _Mem(7, "similar prior incident")
    ...
    assert res["matched"] is True
    assert res["memory_id"] == 7
    assert res["match_type"] == "semantic"
    assert res["similarity"] == round(1 - 0.1, 4)
    assert res["candidates"][0]["memory_id"] == 7


async def test_get_memory_semantic_rejected_above_threshold():
    mem = _Mem(7, "distant prior incident")
    async def fake_embed(text):
        return [0.5] * EMBEDDING_DIM
    ...
    assert res["candidates"] == [
        {"memory_id": 7, "distance": 0.9, "similarity": 0.1}
    ]


async def test_get_memory_falls_back_when_no_embedding_wired():
    sess = _FakeSession()
    res = await get_memory(sess, 1, query_text="x")
    assert res["matched"] is False
    assert res["match_type"] == "none"


async def test_get_memory_exact_still_works_without_embedding():
    mem = _Mem(9, "exact via dedupe")
    sess = _FakeSession(exact_memory=mem)
    res = await get_memory(sess, 1, query_text="x", dedupe_key="k")
    assert res["matched"] is True
    assert res["memory_id"] == 9
    assert res["match_type"] == "exact"


async def test_get_memory_embed_failure_falls_back():
    async def fake_embed(text):
        return None
...
async def test_semantic_search_ranks_by_cosine_distance():
    # Query vector points along [1, 0]; mem1 is identical, mem2 orthogonal,
    # mem3 opposite.
    query = [1.0, 0.0]
    mem1 = _Mem(1, "identical", [1.0, 0.0])
    mem2 = _Mem(2, "orthogonal", [0.0, 1.0])
    mem3 = _Mem(3, "opposite", [-1.0, 0.0])
    mem_null = _Mem(4, "no embedding", None)
    sess = _FakeSession(rows=[mem3, mem_null, mem1, mem2])
    ...
    assert ids == [1, 2, 3]
    # Distances: mem1=0, mem2=1, mem3=2.
...
async def test_semantic_search_respects_top_k():
    query = [1.0, 0.0, 0.0]
    rows = [_Mem(i, f"m{i}", [float(i), 0.0, 0.0]) for i in range(1, 6)]
